...
for label, pw in cases:
    p = dict(base, platter_width=pw, stencil_lip=(100 - pw) / 2)
    out = Path(rf"C:\Users\Hszn\AppData\Local\Temp\verify_{pw}.stl")
    generate_to_file(p, "insert", out, "stl")
    data = out.read_bytes()
    h = hashlib.sha256(data).hexdigest()[:12]
    sz = len(data)
    hashes.append((label, h, sz))
    print(f"{label}: sha256={h} size={sz} bytes")

print()
print("=== STL hash 是否都不同?(应该 4 个全不同) ===")
unique = len(set(h for _, h, _ in hashes))
print(f"不同 hash 数: {unique}/4 {'✓' if unique == 4 else '✗ 重合,模型没变化!'}")
